...
static void load(Type *ty) {
  if (ty->kind == TypeKind::TY_ARRAY || ty->kind == TypeKind::TY_STRUCT ||
...
static void emit_data(Obj *prog) {
  for (Obj *var = prog; var; var = var->next) {
    ...
    if (var->init_data) {
      for (int i = 0; i < var->ty->size; i++) println("  .byte %d", var->init_data[i]);
    } else {
...
static void emit_text(Obj *prog) {
  ...
  for (Obj *fn = prog; fn; fn = fn->next) {
    ...
    int i = 0;
    for (Obj *var = fn->params; var; var = var->next) store_gp(i++, var->offset, var->ty->size);
