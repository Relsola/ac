...
static void copy_ret_buffer(Obj *var) {
  ...
  if (has_flonum1(ty)) {
    ...
    for (int i = 0; i < std::min(8, ty->size); i++) {
  ...
  if (ty->size > 8) {
    if (has_flonum2(ty)) {
      ...
      for (int i = 8; i < std::min(16, ty->size); i++) {
...
static void gen_expr(Node *node) {
  ...
  switch (node->kind) {
    ...
    case NodeKind::ND_FUNCALL: {
      if (node->lhs->kind == NodeKind::ND_VAR && !strcmp(node->lhs->var->name, "alloca")) {
        gen_expr(node->args);
        println("  mov %%rax, %%rdi");
        builtin_alloca();
        return;
  ...
  switch (node->kind) {
    ...
    case NodeKind::ND_SWITCH:
      ...
      for (Node *n = node->case_next; n; n = n->case_next) {
        char const *reg = (node->cond->ty->size == 8) ? "%rax" : "%eax";
  ...
  for (Obj *fn = prog; fn; fn = fn->next) {
    ...
    for (Obj *var = fn->params; var; var = var->next) {
      ...
      switch (ty->kind) {
        ...
        case TypeKind::TY_UNION:
          if (has_flonum(ty, 0, 8, 0))
            store_fp(fp++, var->offset, std::min(8, ty->size));
          else
            store_gp(gp++, var->offset, std::min(8, ty->size));
