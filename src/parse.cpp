...
static Node *new_vla_ptr(Obj *var, Token *tok) {
  Node *node = new_node(NodeKind::ND_VLA_PTR, tok);
  node->var = var;
  return node;
}
...
static Type *declspec(Token **rest, Token *tok, VarAttr *attr) {
  ...
  while (is_typename(tok)) {
    ...
    if (tok->equal("struct") || tok->equal("union") || tok->equal("enum") || tok->equal("typeof") ||
        ty2) {
      if (counter) break;
...
static Type *func_params(Token **rest, Token *tok, Type *ty) {
  ...
  while (!tok->equal(")")) {
    ...
    if (ty2->kind == TypeKind::TY_ARRAY) {
      ...
      ty2->name = name;
    } else if (ty2->kind == TypeKind::TY_FUNC) {
...
static void array_designator(Token **rest, Token *tok, Type *ty, int *begin, int *end) {
  *begin = const_expr(&tok, tok->next);
  if (*begin >= ty->array_len) error_tok(tok, "array designator index exceeds array bounds");
...
static Member *struct_designator(Token **rest, Token *tok, Type *ty) {
  Token *start = tok;
  tok = tok->skip(".");
  if (tok->kind != TokenKind::TK_IDENT) error_tok(tok, "expected a field designator");
...
static void designation(Token **rest, Token *tok, Initializer *init) {
  ...
  if (tok->equal(".") && init->ty->kind == TypeKind::TY_STRUCT) {
...
static int count_array_init_elements(Token *tok, Type *ty) {
  ...
  while (!consume_end(&tok, tok)) {
    ...
    if (tok->equal("[")) {
      ...
      if (tok->equal("...")) i = const_expr(&tok, tok->next);
      tok = tok->skip("]");
...
static int64_t eval(Node *node, char **label) {
  ...
  if (node->ty->is_flonum()) return eval_double(node);
...
static double eval_double(Node *node) {
  ...
  switch (node->kind) {
    ...
    case NodeKind::ND_CAST:
      if (node->lhs->ty->is_flonum()) return eval_double(node->lhs);
      return eval(node->lhs);
    case NodeKind::ND_NUM:
      return node->fval;
  }
...
static Node *to_assign(Node *binary) {
  ...
  if (binary->lhs->kind == NodeKind::ND_MEMBER) {
    Obj *var = new_lvar("", Type::pointer_to(binary->lhs->lhs->ty));

    Node *expr1 = new_binary(NodeKind::ND_ASSIGN,
                             new_var_node(var, tok),
                             new_unary(NodeKind::ND_ADDR, binary->lhs->lhs, tok),
                             tok);

    Node *expr2 = new_unary(NodeKind::ND_MEMBER,
                            new_unary(NodeKind::ND_DEREF, new_var_node(var, tok), tok),
                            tok);
    expr2->member = binary->lhs->member;

    Node *expr3 = new_unary(NodeKind::ND_MEMBER,
                            new_unary(NodeKind::ND_DEREF, new_var_node(var, tok), tok),
                            tok);
    expr3->member = binary->lhs->member;

    Node *expr4 = new_binary(NodeKind::ND_ASSIGN,
                             expr2,
                             new_binary(binary->kind, expr3, binary->rhs, tok),
                             tok);

    return new_binary(NodeKind::ND_COMMA, expr1, expr4, tok);
  }
...
static Node *new_add(Node *lhs, Node *rhs, Token *tok) {
  ...
  if (lhs->ty->base->kind == TypeKind::TY_VLA) {
    rhs = new_binary(NodeKind::ND_MUL, rhs, new_var_node(lhs->ty->base->vla_size, tok), tok);
    return new_binary(NodeKind::ND_ADD, lhs, rhs, tok);
  }
...
static Node *unary(Token **rest, Token *tok) {
  ...
  if (tok->equal("*")) {
    ...
    add_type(node);
    if (node->ty->kind == TypeKind::TY_FUNC) return node;
    return new_unary(NodeKind::ND_DEREF, node, tok);
  }
...
static Node *funcall(Token **rest, Token *tok, Node *fn) {
  ...
  if (fn->ty->kind != TypeKind::TY_FUNC &&
      (fn->ty->kind != TypeKind::TY_PTR || fn->ty->base->kind != TypeKind::TY_FUNC))
    error_tok(fn->tok, "not a function");

  Type *ty = (fn->ty->kind == TypeKind::TY_FUNC) ? fn->ty : fn->ty->base;
  ...
  while (!tok->equal(")")) {
    ...
    if (param_ty) {
      ...
      arg = new_cast(arg, Type::ty_double);
    }
  ...
  *rest = tok->skip(")");
  ...
  Node *node = new_unary(NodeKind::ND_FUNCALL, fn, tok);
  node->func_ty = ty;
...
static Node *primary(Token **rest, Token *tok) {
  ...
  if (tok->kind == TokenKind::TK_IDENT) {
    ...
    *rest = tok->next;
    ...
    if (tok->next->equal("(")) error_tok(tok, "implicit declaration of a function");
...
static Token *function(Token *tok, Type *basety, VarAttr *attr) {
  ...
  push_scope("__func__")->var =
      new_string_literal(fn->name, Type::array_of(Type::ty_char, strlen(fn->name) + 1));
