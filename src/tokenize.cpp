...
static int read_punct(char *p) {
  ...
  for (int i = 0; i < sizeof(kw) / sizeof(*kw); i++)
    if (startswith(p, kw[i])) return strlen(kw[i]);
...
static bool is_keyword(Token *tok) {
  static char *kw[] = {
      ...
      "__restrict__", "_Noreturn", "float",    "double",   "typeof",   "asm",
  };
...
static Token *read_utf16_string_literal(char *start, char *quote) {
  char *end = string_literal_end(quote + 1);
  uint16_t *buf = new uint16_t[end - start]();
  int len = 0;
  ...
  Token *tok = new_token(TokenKind::TK_STR, start, end + 1);
  tok->ty = Type::array_of(Type::ty_ushort, len + 1);
  tok->str = (char *)buf;
  return tok;
}
...
static Token *read_utf32_string_literal(char *start, char *quote, Type *ty) {
  char *end = string_literal_end(quote + 1);
  uint32_t *buf = new uint32_t[end - quote]();
  int len = 0;
...
static bool convert_pp_int(Token *tok) {
  ...
  if (base == 10) {
    if (l && u)
      ...
      ty = (val >> 32) ? Type::ty_ulong : Type::ty_uint;
    else
      ty = (val >> 31) ? Type::ty_long : Type::ty_int;
  } else {
    if (l && u)
      ty = Type::ty_ulong;
    else if (l)
      ty = (val >> 63) ? Type::ty_ulong : Type::ty_long;
    else if (u)
      ty = (val >> 32) ? Type::ty_ulong : Type::ty_uint;
    else if (val >> 63)
      ty = Type::ty_ulong;
...
void convert_pp_tokens(Token *tok) {
  for (Token *t = tok; t->kind != TokenKind::TK_EOF; t = t->next) {
    ...
    else if (t->kind == TokenKind::TK_PP_NUM)
      convert_pp_number(t);
  }
}
...
Token *tokenize(File *file) {
  ...
  while (*p) {
    ...
    if (isdigit(*p) || (*p == '.' && isdigit(p[1]))) {
      ...
      cur = cur->next = new_token(TokenKind::TK_PP_NUM, q, p);
      continue;
    }
    ...
    // Wide string literal
    if (startswith(p, "L\"")) {
      cur = cur->next = read_utf32_string_literal(p, p + 1, Type::ty_int);
      p += cur->len;
      continue;
    }
    ...
    if (startswith(p, "U\"")) {
      cur = cur->next = read_utf32_string_literal(p, p + 1, Type::ty_uint);
      p += cur->len;
      continue;
    }
    ...
    // Character literal
    if (*p == '\'') {
      cur = cur->next = read_char_literal(p, p, Type::ty_int);
    ...
    if (startswith(p, "u'")) {
      cur = cur->next = read_char_literal(p, p + 1, Type::ty_ushort);
      cur->val &= 0xffff;
      p += cur->len;
      continue;
    }
    ...
    if (startswith(p, "U'")) {
      cur = cur->next = read_char_literal(p, p + 1, Type::ty_uint);
      p += cur->len;
      continue;
    }
    ...
    if (startswith(p, "L'")) {
      cur = cur->next = read_char_literal(p, p + 1, Type::ty_int);
      p += cur->len;
      continue;
    }
    ...
    if (ident_len) {
      cur = cur->next = new_token(TokenKind::TK_IDENT, p, p + ident_len);
      p += cur->len;
      continue;
    }
