...
struct MacroParam {
  MacroParam *next = nullptr;
  char *name = nullptr;
};

struct MacroArg {
  MacroArg *next = nullptr;
  char *name = nullptr;
...
static Token *skip_cond_incl2(Token *tok) {
  while (tok->kind != TokenKind::TK_EOF) {
    if (is_hash(tok) &&
        (tok->next->equal("if") || tok->next->equal("ifdef") || tok->next->equal("ifndef"))) {
...
static Token *read_const_expr(Token **rest, Token *tok) {
  ...
  while (tok->kind != TokenKind::TK_EOF) {
    ...
    if (tok->equal("defined")) {
      ...
      if (tok->kind != TokenKind::TK_IDENT) error_tok(start, "macro name must be an identifier");
      Macro *m = find_macro(tok);
      tok = tok->next;

      if (has_paren) tok = tok->skip(")");
...
static long eval_const_expr(Token **rest, Token *tok) {
  ...
  for (Token *t = expr; t->kind != TokenKind::TK_EOF; t = t->next) {
...
static MacroArg *read_macro_arg_one(Token **rest, Token *tok, bool read_rest) {
  ...
  for (;;) {
    if (level == 0 && tok->equal(")")) break;
    if (level == 0 && !read_rest && tok->equal(",")) break;

    if (tok->kind == TokenKind::TK_EOF) error_tok(tok, "premature end of input");
...
static char *join_tokens(Token *tok, Token *end) {
  ...
  int len = 1;
  for (Token *t = tok; t != end && t->kind != TokenKind::TK_EOF; t = t->next) {
  ...
  while (tok->kind != TokenKind::TK_EOF) {
      ...
      if (arg) {
        if (arg->tok->kind != TokenKind::TK_EOF) {
          *cur = *paste(cur, arg->tok);
          for (Token *t = arg->tok->next; t->kind != TokenKind::TK_EOF; t = t->next)
    ...
    if (arg && tok->next->equal("##")) {
      ...
      if (arg->tok->kind == TokenKind::TK_EOF) {
        ...
        if (arg2) {
          for (Token *t = arg2->tok; t->kind != TokenKind::TK_EOF; t = t->next)
            cur = cur->next = copy_token(t);
        } else {
    ...
    if (arg) {
      ...
      for (; t->kind != TokenKind::TK_EOF; t = t->next) cur = cur->next = copy_token(t);
      tok = tok->next;
      continue;
    }
...
static bool expand_macro(Token **rest, Token *tok) {
  ...
  if (m->is_objlike) {
    ...
    for (Token *t = body; t->kind != TokenKind::TK_EOF; t = t->next) t->origin = tok;
  ...
  for (Token *t = body; t->kind != TokenKind::TK_EOF; t = t->next) t->origin = macro_token;
  *rest = append(body, tok->next);
...
static char *read_include_filename(Token **rest, Token *tok, bool *is_dquote) {
  ...
  if (tok->equal("<")) {
    ...
    for (; !tok->equal(">"); tok = tok->next)
      if (tok->at_bol || tok->kind == TokenKind::TK_EOF) error_tok(tok, "expected '>'");
...
static void read_line_marker(Token **rest, Token *tok) {
  ...
  if (tok->kind != TokenKind::TK_NUM || tok->ty->kind != TypeKind::TY_INT)
    error_tok(tok, "invalid line marker");
  start->file->line_delta = tok->val - start->line_no;

  tok = tok->next;
  if (tok->kind == TokenKind::TK_EOF) return;

  if (tok->kind != TokenKind::TK_STR) error_tok(tok, "filename expected");
...
static void join_adjacent_string_literals(Token *tok) {
  ...
  for (Token *tok1 = tok; tok1->kind != TokenKind::TK_EOF;) {
    ...
    for (Token *t = tok1->next; t->kind == TokenKind::TK_STR; t = t->next) {
    ...
    if (basety->size > 1)
      for (Token *t = tok1; t->kind == TokenKind::TK_STR; t = t->next)
        if (t->ty->base->size == 1) *t = *tokenize_string_literal(t, basety);

    while (tok1->kind == TokenKind::TK_STR) tok1 = tok1->next;
  }
  ...
  for (Token *tok1 = tok; tok1->kind != TokenKind::TK_EOF;) {
    ...
    *tok1 = *copy_token(tok1);
    tok1->ty = Type::array_of(tok1->ty->base, len);
    tok1->str = buf;
    tok1->next = tok2;
    tok1 = tok2;
  }
